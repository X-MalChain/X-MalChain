...
def find_sensitive_api(match_nodes):
    ...
    for node_id in match_nodes:
        ...
        node_obj = models.augmenTestNode.objects.get(id=str(node_id))
        ...
        if api_list != '' and api_list != " ":
            ...
            if len(apis_list) > 0:
                for api_id in apis_list:
                    api_name = models.augmenTestAPi.objects.get(apiID=int(api_id)).apiName
                    try:
    ...
    if len(augment_nodes) > 0:
        ...
        for key, value in ret.items():  # 按照父函数排序后的结果
            ...
            for one in judge:
                ...
                if source_kg_node == 0:  # 说明在原数据库中不存在，因此需要先添加
                    ...
                    if len(pers) > 0:
                        for per in pers:
                            try:
                                per_ans = models.augmenTestPer.objects.get(perName=per)
                                if per_ans:
                                    per_list = per_list + str(per_ans.perID) + ','
                            except Exception as e:  # 现有数据库中没有这条权限
                                print('查询per id失败：', per)
                                now_len = len(list(models.augmenTestPer.objects.values()))
                                models.augmenTestPer.objects.create(perName=per, perID=now_len + 1)
                                per_ans = models.augmenTestPer.objects.get(perName=per)
                    ...
                    if len(apis) > 0:
                        for api in apis:
                            try:
                                api_ans = models.augmenTestAPi.objects.get(apiName=api.strip())
                                if api_ans:
                                    api_list = api_list + str(api_ans.apiID) + ','
                            except Exception as e:  # 现有数据库中没有这条API
                                # print('查询api id失败：', api)
                                now_len = len(list(models.augmenTestAPi.objects.values())) + 6  # 因为之前删除过一些API
                                models.augmenTestAPi.objects.create(apiName=api, apiID=now_len + 1)
                                api_ans = models.augmenTestAPi.objects.get(apiName=api)
                                ans_id = api_ans.apiID
                                models.augmenTestAPi.objects.filter(apiID=ans_id).update(id=ans_id)  # 设置node ID的值
                    ...
                    try:
                        q = models.augmenTestNode.objects.filter(actionName=node_name)
                        if q:  # 已经存在
                            one['source_kg_node'] = q[0].nodeID
                            path.append(one['source_kg_node'])
                        else:
                            # 创建新的节点
                            kg_count = len(list(models.augmenTestNode.objects.values()))
                            one['source_kg_node'] = kg_count + 1
                            source_kg_node = one['source_kg_node']
                            mark = one['mark']
                            path.append(source_kg_node)
                            # print('api list:', api_list)
                            models.augmenTestNode.objects.create(nodeID=source_kg_node, actionName=node_name,
                                                                 perList=per_list,
                                                                 apiList=api_list,
                                                                 mark=mark)
                            models.augmenTestNode.objects.filter(nodeID=source_kg_node).update(id=source_kg_node)
                    except Exception as e:
                        # print(e)
                        pass
                else:
            ...
            while i < len(path) - 1:
                source = path[i]
                target = path[i + 1]
                ans = models.augmenTestRel.objects.filter(Q(sourceID=source) & Q(targetID=target))
                if ans:
                    pass  # 该关系已经存在
                else:
                    counts = len(list(models.augmenTestRel.objects.values()))
                    models.augmenTestRel.objects.create(sourceID=source, targetID=target)
                    models.augmenTestRel.objects.filter(Q(sourceID=source) & Q(targetID=target)).update(id=counts + 1)
                i = i + 1

    return new_path


def auto_augment_self():
    """
    ...
    # 读取匹配的输出结果
    file_path = '../detect/output/match_report1_augment.txt'
    ...
    apks_location = '/media/wuyang/WD_BLACK/AndroidMalware/malware_test_genome'
    logs = '../detect/output/augment_logs.txt'

    # 输出结果
    output_path = '../detect/output/augment_report1.txt'
    # 清空输出报告
...
# a=[5,8,37,42,25,28,43,7,57,39,19,31,53,11,23,36,4,46,41,52,50,51,3,30]
# print('len:',len(a))

"""
以下是为了不同版本的扩充后的知识图谱的备份
"""
...
def copy_kg_db_v1():
    ...
    将经过amd扩充后的图谱备份
    """
    models.augmentAMDNode.objects.all().delete()
    models.augmentAMDRel.objects.all().delete()
    models.augmentAMDAPi.objects.all().delete()
    models.augmentAMDPer.objects.all().delete()

    nodes = list(models.augmenTestNode.objects.values())
    a = 0
    for one in nodes:
        a = a + 1
        models.augmentAMDNode.objects.create(nodeID=one['nodeID'], actionName=one['actionName'], perList=one['perList'],
                                             apiList=one['apiList'])
        models.augmentAMDNode.objects.filter(nodeID=one['nodeID']).update(id=a)  # 设置node ID的值
    ...
    for one in rels:
        ...
        b = b + 1
        models.augmentAMDRel.objects.create(sourceID=one['sourceID'], sourceAct=one['sourceAct'],
                                            targetID=one['targetID'], targetAct=one['targetAct'],
                                            relation=one['relation'])
        try:
            models.augmentAMDRel.objects.filter(Q(sourceID=one['sourceID']) & Q(targetID=one['targetID'])).update(id=b)
        except Exception as e:
            print(e)
    ...
    for one in apis:
        c = one['apiID']
        models.augmentAMDAPi.objects.create(apiID=one['apiID'], apiName=one['apiName'], inLevel=one['inLevel'],
                                            outLevel=one['outLevel'], addList=one['addList'], repList=one['repList'])
        models.augmentAMDAPi.objects.filter(apiID=one['apiID']).update(id=c)

    pers = list(models.augmenTestPer.objects.values())
    d = 0
    for one in pers:
        d = d + 1
        models.augmentAMDPer.objects.create(perID=one['perID'], perName=one['perName'], inLevel=one['inLevel'],
                                            outLevel=one['outLevel'])
        models.augmentAMDPer.objects.filter(perID=one['perID']).update(id=d)
...
def copy_kg_db_v2():
    """
    :function 复制有关知识图谱的两张数据表（节点表和关系表）+api表 到备用数据表中
    将经过amd+drebin扩充后的图谱备份
    """
    models.augmentNode2.objects.all().delete()
    models.augmentRel2.objects.all().delete()
    models.augmentAPi2.objects.all().delete()
    models.augmentPer2.objects.all().delete()

    nodes = list(models.augmenTestNode.objects.values())
    a = 0
    for one in nodes:
        a = a + 1
        models.augmentNode2.objects.create(nodeID=one['nodeID'], actionName=one['actionName'], perList=one['perList'],
                                           apiList=one['apiList'])
        models.augmentNode2.objects.filter(nodeID=one['nodeID']).update(id=a)  # 设置node ID的值
    ...
    for one in rels:
        ...
        # print('current_id:', current_id)
        b = b + 1
        models.augmentRel2.objects.create(sourceID=one['sourceID'], sourceAct=one['sourceAct'],
                                          targetID=one['targetID'], targetAct=one['targetAct'],
                                          relation=one['relation'])
        try:
            models.augmentRel2.objects.filter(Q(sourceID=one['sourceID']) & Q(targetID=one['targetID'])).update(id=b)
        except Exception as e:
            print(e)
            # print(str(one['sourceID'])+str(one['targetID']))
    ...
    for one in apis:
        c = one['apiID']
        models.augmentAPi2.objects.create(apiID=one['apiID'], apiName=one['apiName'], inLevel=one['inLevel'],
                                          outLevel=one['outLevel'], addList=one['addList'], repList=one['repList'])
        models.augmentAPi2.objects.filter(apiID=one['apiID']).update(id=c)

    pers = list(models.augmenTestPer.objects.values())
    d = 0
    for one in pers:
        d = d + 1
        models.augmentPer2.objects.create(perID=one['perID'], perName=one['perName'], inLevel=one['inLevel'],
                                          outLevel=one['outLevel'])
        models.augmentPer2.objects.filter(perID=one['perID']).update(id=d)
...
def copy_kg_db_v3():
    """
    :function 复制有关知识图谱的两张数据表（节点表和关系表）+api表 到备用数据表中
    将经过amd+drebin+genome扩充后的图谱备份
    """
    models.augmentNode3.objects.all().delete()
    models.augmentRel3.objects.all().delete()
    models.augmentAPi3.objects.all().delete()
    models.augmentPer3.objects.all().delete()

    nodes = list(models.augmenTestNode.objects.values())
    a = 0
    for one in nodes:
        a = a + 1
        models.augmentNode3.objects.create(nodeID=one['nodeID'], actionName=one['actionName'], perList=one['perList'],
                                           apiList=one['apiList'])
        models.augmentNode3.objects.filter(nodeID=one['nodeID']).update(id=a)  # 设置node ID的值
    ...
    for one in rels:
        ...
        # print('current_id:', current_id)
        b = b + 1
        models.augmentRel3.objects.create(sourceID=one['sourceID'], sourceAct=one['sourceAct'],
                                          targetID=one['targetID'], targetAct=one['targetAct'],
                                          relation=one['relation'])
        try:
            models.augmentRel3.objects.filter(Q(sourceID=one['sourceID']) & Q(targetID=one['targetID'])).update(id=b)
        except Exception as e:
            print(e)
            # print(str(one['sourceID'])+str(one['targetID']))
    ...
    for one in apis:
        c = one['apiID']
        models.augmentAPi3.objects.create(apiID=one['apiID'], apiName=one['apiName'], inLevel=one['inLevel'],
                                          outLevel=one['outLevel'], addList=one['addList'], repList=one['repList'])
        models.augmentAPi3.objects.filter(apiID=one['apiID']).update(id=c)

    pers = list(models.augmenTestPer.objects.values())
    d = 0
    for one in pers:
        d = d + 1
        models.augmentPer3.objects.create(perID=one['perID'], perName=one['perName'], inLevel=one['inLevel'],
                                          outLevel=one['outLevel'])
        models.augmentPer3.objects.filter(perID=one['perID']).update(id=d)
...
def copy_kg_db_In():
    ...
    将经过amd+drebin+genome扩充后的图谱备份
    """
    # models.augmentNodeIn.objects.all().delete()
    # models.augmentRelIn.objects.all().delete()
    models.augmentAPiIn.objects.all().delete()
    # models.augmentPerIn.objects.all().delete()

    # nodes = list(models.augmenTestNode.objects.values())
    # a = 0
    # for one in nodes:
    #     a = a + 1
    #     models.augmentNodeIn.objects.create(nodeID=one['nodeID'], actionName=one['actionName'], perList=one['perList'],
    #                                          apiList=one['apiList'])
    #     models.augmentNodeIn.objects.filter(nodeID=one['nodeID']).update(id=a)  # 设置node ID的值
    ...
    #     # print('current_id:', current_id)
    #     b = b + 1
    #     models.augmentRelIn.objects.create(sourceID=one['sourceID'], sourceAct=one['sourceAct'],
    #                                         targetID=one['targetID'], targetAct=one['targetAct'],
    #                                         relation=one['relation'])
    #     try:
    #         models.augmentRelIn.objects.filter(Q(sourceID=one['sourceID']) & Q(targetID=one['targetID'])).update(id=b)
    #     except Exception as e:
    #         print(e)
            # print(str(one['sourceID'])+str(one['targetID']))
    ...
    for one in apis:
        c = one['apiID']
        models.augmentAPiIn.objects.create(apiID=one['apiID'], apiName=one['apiName'], inLevel=one['inLevel'],
                                            outLevel=one['outLevel'], addList=one['addList'], repList=one['repList'])
        models.augmentAPiIn.objects.filter(apiID=one['apiID']).update(id=c)

    # pers = list(models.augmenTestPer.objects.values())
    # d = 0
    # for one in pers:
    #     d = d + 1
    #     models.augmentPerIn.objects.create(perID=one['perID'], perName=one['perName'], inLevel=one['inLevel'],
    #                                        outLevel=one['outLevel'])
    #     models.augmentPerIn.objects.filter(perID=one['perID']).update(id=d)
...
def optim_Relation():
    ...
    for one in model:
        newID = newID + 1
        oldRelID = one['id']
        if newID != oldRelID:  # 只更新那些有问题的
            models.augmentRelIn.objects.filter(id=oldRelID).update(id=newID)  # 更新ID
