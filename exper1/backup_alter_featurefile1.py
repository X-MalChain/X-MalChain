...
from common.models import augmenTestAPi, PerTest, augmenTestNode, augmenTestRel

kg_permissions = []  # all permissions in kg/database
kg_apis = []  # all apis in kg/database
apis_from_kg = []  # 那些已经确立为节点特征的apis
...
def apk_map_kg_main():
    ...
    with open(report_path, "a", encoding='utf-8') as report:
        ...
        for f in files:  # f形如D:/input/apk01.apk
            ...
            for apiId in apiIdList:
                for one in kgList:
                    ...
                    if apiStr == '':
                        ...
                        if apiStr.find(str(apiId)) != -1:
                            partNode.append(one['id'])
            ...
            #         tmp1 = line.split(';')
            #         tmp = ';'.join(tmp1)  # tmp为按顺序出现的api
            #         ans = ApiTest.objects.filter(apiName=tmp)
            #         if ans:
            #             ans = list(ans)
            #             for one in ans:
            #                 apiIdList.append(one.id)
