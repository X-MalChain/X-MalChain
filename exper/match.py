...
def extract_features(apk_name, apk_path):
    """
...
def apk_map_kg_main():
    ...
    apis_from_test = get_apis_from_test()  # 从ApiTest表中读取api name
    ...
    kgModel = augmenTestNode.objects.values()  # 获取图谱的数据
    kgList = list(kgModel)
    ...
    with open(report_path, "a", encoding='utf-8') as report:
        ...
        for f in files:  # f形如D:/input/apk01.apk
            ...
            extract_features(apk_name, f)  # 提取特征,生成特征文件
            # 写入report
            ...
            for row in apk_file.readlines():
                line = row.strip()
                if line == '' or line.find('entrypoint') != -1:
                    pass
                else:
            ...
            match_report_ans.append({'apk_name': apk_name, 'match_path': str(pathList)})
            ...
            with open(match_report_path, 'w', encoding='utf-8', newline="") as f:
                for one in match_report_ans:
                    f.write(json.dumps(one, indent=4, ensure_ascii=False)) # 字典转为字符串
                    f.write(';')
